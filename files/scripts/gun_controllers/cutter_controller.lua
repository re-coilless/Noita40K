...
if( beam_id == nil or not( EntityGetIsAlive( beam_id ))) then
	beam_id = EntityLoad( "mods/Noita40K/files/entities/projectiles/beam_cutter_visual.xml", w_x, w_y )
...
elseif( state and not( overheated )) then
	...
	for m = 0,amount - 1 do
		...
		EntityLoad( "mods/Noita40K/files/entities/projectiles/beam_cutter_physical.xml", beam_eater_x, beam_eater_y )
	end
	...
	local hit_action = function( hooman, deadman, k, beam_part_x, beam_part_y, hitbox_dmg_scale )
		...
		EntityInflictDamage( deadman, 0.6*hitbox_dmg_scale, "DAMAGE_MATERIAL", "[BURN, HERETIC]", "NORMAL", 0, 0, hooman, beam_part_x, beam_part_y, 0 )
	end
