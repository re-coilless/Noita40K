...
if( LMB_down ) then
	if( not( LMB_pressed )) then
		GamePlaySound( "mods/Noita40K/files/sfx/40K.bank", "fx/ui/holo_button", i_x, i_y )
...
if( RMB_down ) then
	if( not( RMB_pressed )) then
		if( osculant_count > 0 ) then
			GamePlaySound( "mods/Noita40K/files/sfx/40K.bank", "items/osculant_device/extract", i_x, i_y )
			...
			GamePlaySound( "mods/Noita40K/files/sfx/40K.bank", "fx/ui/holo_failure", i_x, i_y )
...
if( matter_count > 5000 ) then
	...
	GamePlaySound( "mods/Noita40K/files/sfx/40K.bank", "items/osculant_device/synthesize", i_x, i_y )
	ComponentSetValue2( osculant_storage, "value_int", osculant_count + 1 )
