...
if( not( pen.is_inv_active())) then
	...
	function mode_controller()
		...
		if( clicked ) then
			if( explorator_mode ) then
				GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/exp_mode_off", char_x, char_y )
				new_console_entry( hooman, "[COMBAT_MODE]", 0 )
				ComponentSetValue2( ui_mode_storage, "value_bool", false )
			else
				GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/exp_mode_on", char_x, char_y )
	...
	function dendrites_controller()
		...
		if( clicked ) then
			if( dendrites_active ) then
				...
				GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/forward", char_x, char_y )
				...
				if( not( capture_activated )) then
					if( t_dist < 10 ) then
						if( target_entity ~= captured_entity ) then
							GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/capture_mode_on", char_x, char_y )
							new_console_entry( hooman, "Target "..target_entity.." captured.", 0 )
							ComponentSetValue2( captured_storage, "value_int", target_entity )
						end
					else
						GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/capture_mode_off", char_x, char_y )
	...
	function servoskull_interface()
		...
		if( not( skull_menu_state )) then
			...
			if( clicked ) then
				...
				GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/menu_open", char_x, char_y )
			...
			uid = window_normal( gui, uid, pic_x, pic_y, pic_z, 114, 80, 2, function()
				...
				GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/menu_close", char_x, char_y )
						...
						for i = 1,5 do
							...
							if( r_clicked ) then
								if( captured_entity ~= 0 and slot_state >= 0 ) then
									...
									GamePlaySound( "mods/Noita40K/files/40K.bank", "fx/ui/holo_failure", char_x, char_y )
