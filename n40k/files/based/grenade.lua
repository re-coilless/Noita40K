    ...
    return function( info )
        ...
        if( r_clicked ) then
            ...
            if( fuse < 0 ) then
                ...
                pen.play_sound( pen.S.N40K.ITEM_PIN_PULL, x, y )
...
function wake_up_waiting_threads()
    ...
    local dims = pen.new.text_shad( pic_x, pic_y - 15, pen.Z.WORLD_FRONT, text, {
        is_centered_x = true, color = pen.P.N40K.HOLO_3, color_shadow = pen.P.N40K.HOLO_1 })
    -- local glow_id = pen.hew.glow( entity_id, pic_x - 1, pic_y - 9,
    --     pen.Z.MAIN_FRONT + 0.1, dims[1], dims[2], pen.P.N40K.HOLO_3, 0.75 )
