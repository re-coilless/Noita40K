return function( info )
    ...
    local data = { m = 0.15, is_debugging = false, dmg = 2/25 }
    if( may_swing and ( xD.Controls.lmb[3] or memo.swing_start )) then
        ...
        elseif( memo.swing_done and xD.Controls.lmb[3]) then
            ...
            data.drift = { r = 80, x = 3 + shake, y = 2 + shake, a = { "wgt", 0.25, true }, m = 0.75 }
        else
        ...
        pen.play_sound( pen.S.N40K.CHAINSWORD_CUT, x, y )
        if( is_cutting ) then pen.play_sound( pen.S.N40K.CHAINSWORD_CUT_, x, y ) end
    ...
    data.on_hit = function( hit_id, hit_x, hit_y, dmg_mult, k )
        local is_metal = false --if got armor or ragdoll is made from metal
        pen.play_sound( pen.S.N40K[ "CHAINSWORD_TEAR"..( is_metal and "_M" or "" )], hit_x, hit_y )
    ...
    data.on_active = function( hooman, x, y, r, length )
        for i = 1,( pen.c.sword_cutting and 3 or 1 ) do
            pen.magic_shooter( hooman, "mods/n40k/files/items/rounds/effect_sword_chain.xml",
    ...
    ComponentSetValue2( pics[1], "image_file", "mods/n40k/files/items/weapons/sword_chain_"..anim..".png" )
    EntityRefreshSprite( info.id, pics[1])
    ComponentSetValue2( pics[2], "image_file", "mods/n40k/files/items/weapons/sword_chain_"..anim.."_heat.png" )
    EntityRefreshSprite( info.id, pics[2])

    pen.play_sound( pen.S.N40K.CHAINSWORD_IDLE, x, y )
