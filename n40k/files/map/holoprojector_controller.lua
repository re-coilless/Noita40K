...
if( broken_cooldown < frame_num ) then
	...
	if( math.sqrt( v_x^2 + v_y^2 ) < 10 and math.deg( math.abs( h_r )) < 15 and math.abs( a_v ) < 0.5 ) then
		if( is_broken ) then
			GamePlaySound( "mods/n40k/files/n40k.bank", "fx/short_circuit_alt", h_x, h_y )
			EntityRemoveTag( entity_id, "broken" )
			EntitySetComponentsWithTagEnabled( entity_id, "active", true )
			is_updated = true
		end
	elseif( not( is_broken )) then
		GamePlaySound( "mods/n40k/files/n40k.bank", "fx/short_circuit", h_x, h_y )
	...
	if( hooman ~= nil ) then
		...
		if( dist < 50 ) then
			...
			if( is_active ) then
				...
				uid, clicked, r_clicked = new_button( gui, uid, pic_x + 182, pic_y + 4, pic_z, "mods/n40k/files/pics/gui_gfx/quest_menu_button_holo_close.png" )
				if( clicked ) then
					GamePlaySound( "mods/n40k/files/n40k.bank", "fx/ui/holo_button", h_x, h_y )
