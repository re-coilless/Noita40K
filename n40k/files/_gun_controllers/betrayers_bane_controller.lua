...
local diameter = 20
local full_power = 0.3

if( beam_id == nil or not( EntityGetIsAlive( beam_id ))) then
	beam_id = EntityLoad( "mods/n40k/files/entities/projectiles/beam_combimelta_visual.xml", w_x, w_y )
	EntityAddChild( wand_id, beam_id )
end

if( mana_current < 25 ) then
	if( not( overheated )) then
		ComponentSetValue2( abil_comp, "mNextFrameUsable", frame_num + 150 )
		GamePlaySound( "mods/n40k/files/n40k.bank", "guns/reloads/overheat_start", w_x, w_y )
		ComponentSetValue2( storage_overheated, "value_bool", true )
	end
elseif( RMB_down and not( overheated ) and not( EntityHasTag( hooman, "twin_linked" ))) then
	GameEntityPlaySoundLoop( wand_id, "melta_is_going", 1.0 )
	ComponentSetValue2( abil_comp, "mana", mana_current - 20 )
	...
	if( hit ) then
		l_x = get_sign( l_x )*math.max( math.abs( hit_x - barrel_x ), math.abs( l_x*full_power ))
		l_y = get_sign( l_y )*math.max( math.abs( hit_y - barrel_y ), math.abs( l_y*full_power ))
	...
	if( lenght > 0 ) then
		...
		for m = 0,amount - 1 do
			...
			EntityLoad( "mods/n40k/files/entities/projectiles/beam_combimelta_physical.xml", beam_eater_x, beam_eater_y )
		end
		...
		local hit_action = function( hooman, deadman, k, beam_part_x, beam_part_y, hitbox_dmg_scale )
			...
			EntityInflictDamage( deadman, 0.3*hitbox_dmg_scale, "DAMAGE_MATERIAL", "[BURN, HERETIC]", "NORMAL", 0, 0, hooman, beam_part_x, beam_part_y, 0 )
		end
...
if( overheated and delay_frame <= frame_num ) then
	GamePlaySound( "mods/n40k/files/n40k.bank", "guns/reloads/overheat_end", w_x, w_y )
