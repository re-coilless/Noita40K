...
if( not( ModSettingGetNextValue( "n40k.DISABLE_SHADERS" ))) then
	set_shader( hooman, "warpfire_edge_effect", true, 1 )
end

if( not( scaled )) then
	GamePlaySound( "mods/n40k/files/n40k.bank", "fx/status_effects/warpfire_start", hb_x, hb_y )
...
if( status ) then
	if( max_hp == current_hp ) then
		...
		EntityLoad( "mods/n40k/files/entities/emitters/explosion_warpfire.xml", hb_x, hb_y )
		EntityKill( effect_id )
	else
