...
// Noita40K stuff (inworld effects) ===========================================================================
	...
	if( refractor_effect[2] > 0 )
	{
		float R = refractor_effect[2];
		float h = refractor_effect[3];
		...
		vec2 refractVecR = vec2( 0, 0 );
		vec2 refractVecG = vec2( 0, 0 );
		vec2 refractVecB = vec2( 0, 0 );
		if( dist < hr )
		{
			float iorOffset = sqrt( dist )*0.00015;
			
			vec2 view = vec2( 0.5, 0.5 );
			vec2 normal = vec2( 0.5, 0.5 );
			refractVecR = refract( view, normal, 1 - iorOffset );
			refractVecG = refract( view, normal, 1 );
			refractVecB = refract( view, normal, 1 + iorOffset );
		}
...
// Noita40K stuff (overlays) ==================================================================================
	...
	{
		color = mix( color, vec3( 0.635, 0.376, 0 ), edge_dist/2 );
	}
	
	if( warpfire_edge_effect[0] > 0.0 )
	{
		color = mix( color, vec3( 0.373, 0, 0.714 ), edge_dist/3 );
	}
	
	if( chogorian_edge_effect[0] > 0.0 )
	{
		color = mix( color, vec3( 0.243, 0, 0.039 ), edge_dist*chogorian_edge_effect[0] );
	}
