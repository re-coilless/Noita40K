...
local anim_active = "mods/n40ke_bss/files/pics/guns_gfx/seraphim_chainsword_active.xml"
local anim_passive = "mods/n40ke_bss/files/pics/guns_gfx/seraphim_chainsword_passive.xml"
local anim_current = ComponentGetValue2( sprite_comp, "image_file" )

local diameter = 12

--attaching the beam spot to simplify getting its origin
...
if( state ) then
	...
	--this will be executed on every successfull hitbox hit
	local hit_action = function( hooman, deadman, k, beam_part_x, beam_part_y, hitbox_dmg_scale )
		...
		if( EntityHasTag( deadman, "robot" ) or EntityHasTag( deadman, "heavy_armoured" )) then
			...
			EntityInflictDamage( deadman, 0.01*hitbox_dmg_scale, "DAMAGE_PHYSICS_BODY_DAMAGED", "[RIP AND TEAR]", "NONE", 0, 0, hooman, beam_part_x, beam_part_y, 0 )
		else
			...
			EntityInflictDamage( deadman, 0.03*hitbox_dmg_scale, "DAMAGE_SLICE", "[RIP AND TEAR]", fx_type, 0, 0, hooman, beam_part_x, beam_part_y, 0 )
		end
	end
