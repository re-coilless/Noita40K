return function( info )
    ...
    local sfx = { "mods/Noita40K/files/40K.bank", "items/guns/sollex" }
    if( xD.active_item ~= gun_id ) then
        if( pen.c.sword_state[ gun_id ].active ) then
            pen.play_sound({ sfx[1], sfx[2].."/die" }, blade_x, blade_y ) end
    ...
    local data = { dmg = 0.07, dmg_type = "DAMAGE_MATERIAL", dmg_msg = "sollex", dmg_effect = "NORMAL",
        ...
        point_action = function( data, point_x, point_y, k, is_final )
            ...
            local effect = "mods/Noita40K/files/items/rounds/effect_plasma_small.xml"
    ...
    else pen.play_sound({ sfx[1], sfx[2].."/idle", true }, blade_x, blade_y ) end
    ...
    local blade_path = "mods/Noita40K/files/items/rounds/beam_sollex.xml"
	...
	elseif( pen.c.beam_sfxes[ beam_id ]) then
		...
		pen.play_sound({ sfx[1], sfx[2].."/ignite" }, blade_x, blade_y )
	end
